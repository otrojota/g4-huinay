class ZBootstrapTable extends ZController {
    ...
    async refresh(rowSelector) {
        ...
        if (this.pagination == "none" || this.pagination == "local") {
            this.createWorkingOverlay();
            try {
                this.rows = await this.triggerEvent("getRows");
            } finally {
                this.removeWorkingOverlay();
            }
            if (!this.rows) {
                this.rows = [];
    ...
    repaint() {
        ...
        if (this.rowsInfo) {
            if (!this.totalRows) {
                this.rowsInfo.textContent = "No se encontraron filas";    
            } else {
                if (this.totalRows == 1) this.rowsInfo.textContent = "Se encontró una fila"
                else {
                    if (this.pagination != "none") {
                        this.rowsInfo.textContent = "Filas " + (startRow + 1) + " a " + endRow + " de " + this.totalRows;
                    } else {
                        this.rowsInfo.textContent = "" + this.totalRows + " filas encontradas";
                    }
                }
            }
        }
    ...
    createWorkingOverlay() {
        ...
        let html = 
            `<div class="text-center" style="background-color: black; opacity: 30%; color: white; position:absolute; left: ${rect.left}px; top: ${rect.top}px; width: ${rect.right - rect.left}px; height: ${rect.bottom - rect.top}px; ">
                <i class="fas fa-lg fa-spin fa-spinner" style="margin-top: ${(rect.bottom - rect.top) / 2 - 16}px;"></i>
            </div>`;
        el.innerHTML = html;
        this.overlay =el.firstChild;
